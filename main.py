...
pygame.display.set_caption("太空戰艦")
...
def draw_init():
    ...
    draw_text(screen, '太空飛行戰', 64, WIDTH/2, HEIGHT/4)
...
class Player(pygame.sprite.Sprite):                 #https://ithelp.ithome.com.tw/articles/10232278
    def __init__(self):
        ...
        self.radius = self.rect.width*0.85/2                        #碰撞半徑判斷
    ...
    def update(self):
        ...
        if self.gun > 1 and now - self.guntime > 5000:
            self.gun = 1
            self.guntime = now
...
while running:
    ...
    hits = pygame.sprite.spritecollide(player ,rocks, True, pygame.sprite.collide_circle)      #回傳字典碰到的rock&players
    for hit in hits:
        new_rock()
